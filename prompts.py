...
涉及多家公司比较/对比/排名时(如"A 和 B 谁的 ROE 更高"/"几家银行的净息差"):
- **每家公司分别调用一次 search_report_meta / search_report_content / search_financial**(每次 query 只塞一家公司名)
- 然后综合各家结果做对比分析
- 不要在一次 query 里塞多家公司名(retriever 一次只精准检索一家)

# Edge case 处理

**Observation 多源矛盾**(同一指标在不同 chunk 数字不同):
- 优先级 search_financial(财报底层) > search_report_meta / search_report_content(机构观点)
...
- 评级 / 目标价 / EPS 预测 / 机构观点 → `search_report_meta`(评级摘要)
- 业务分析 / 主题深度 / 海外扩张 / 财务表格 / 现金流细节 → `search_report_content`(正文章节 + 表格)
- 80+ 财务指标(ROE / 毛利率 / 资产负债率 / 杜邦三因子等) → `search_financial`(单公司财报)
- 行业对比 / 同行排名 / 行业均值 → `search_industry`
- 数值计算(同比增长率 / 行业均值 / PE 验算等) → `calculate`
"""
...
def main():
    ...
    steps = [
        ...
        },
        {
            ...
            "tool_name": "search_report_meta",
            "tool_arguments": {"query": "贵州茅台 投资评级 目标价 2025 2026"},
            "tool_call_id": "call_1",
            "observation": "找到 5 条相关研报评级摘要：\n1. [report | 贵州茅台 | 2026-03-03] 评级：买入 目标价 1700 元...",
        },
    ]
